 ...
 *   wp mailodds sync-status                     Show webhook and sync status
 */
...
class MailOdds_CLI {
	...
	public static function register( MailOdds_API $api ) {
		...
		// Sync status command
		WP_CLI::add_command( 'mailodds sync-status', array( $instance, 'sync_status' ) );
	}
	...
	/**
	 * Show webhook and sync status for the connected store.
	 *
	 * Lists all MailOdds WooCommerce webhooks and their delivery status,
	 * shows the last sync timestamp, and reports pending dead letter count
	 * from the MailOdds API.
	 *
	 * ## EXAMPLES
	 *
	 *     wp mailodds sync-status
	 *
	 * @param array $args       Positional arguments.
	 * @param array $assoc_args Named arguments.
	 */
	public function sync_status( $args, $assoc_args ) {
		...
		$connected = get_option( 'mailodds_store_connected', false );

		WP_CLI::line( '--- Store Connection ---' );
		WP_CLI::line( sprintf( '  Store ID:  %s', ! empty( $store_id ) ? $store_id : 'Not set' ) );
		WP_CLI::line( sprintf( '  Connected: %s', $connected ? 'Yes' : 'No' ) );
		WP_CLI::line( '' );

		// WooCommerce webhooks
		WP_CLI::line( '--- WooCommerce Webhooks ---' );

		$catalog = MailOdds::get_instance()->catalog;
		$webhooks = $catalog->get_mailodds_webhooks();

		if ( empty( $webhooks ) ) {
			WP_CLI::line( '  No MailOdds webhooks registered.' );
			if ( ! class_exists( 'WooCommerce' ) ) {
				WP_CLI::line( '  (WooCommerce is not active)' );
			}
		} else {
			WP_CLI\Utils\format_items( 'table', $webhooks, array( 'id', 'topic', 'status', 'failure_count' ) );
		}
		WP_CLI::line( '' );

		// Last sync timestamp from store API
		WP_CLI::line( '--- Last Sync ---' );

		if ( ! empty( $store_id ) ) {
			$store = $this->api->get_store( $store_id );
			if ( is_wp_error( $store ) ) {
				WP_CLI::line( '  Could not fetch store info: ' . $store->get_error_message() );
			} else {
				$last_sync = isset( $store['last_synced_at'] ) ? $store['last_synced_at'] : '';
				WP_CLI::line( sprintf( '  Last synced: %s', ! empty( $last_sync ) ? $last_sync : 'Never' ) );
			}
		} else {
			WP_CLI::line( '  No store connected.' );
		}
		WP_CLI::line( '' );

		// Dead letter / pending deliveries from webhook CLI API
		WP_CLI::line( '--- Pending Webhook Deliveries ---' );

		$deliveries = $this->api->list_webhook_deliveries( array( 'per_page' => 1 ) );
		if ( is_wp_error( $deliveries ) ) {
			WP_CLI::line( '  Could not fetch deliveries: ' . $deliveries->get_error_message() );
		} else {
			$total = isset( $deliveries['total'] ) ? intval( $deliveries['total'] ) : 0;
			WP_CLI::line( sprintf( '  Pending dead letter count: %d', $total ) );
		}
	}
}
